...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('cours').select('*, fiches(count)').eq('user_id', user.id).order('created_at', { ascending: false }),
    ...
    supabase.from('fiches').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('memorized', false).lte('next_review', today),
